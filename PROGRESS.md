...
| 9 | **測試規格** | PI 2026/08/28 提問：專案是否含軟體測試規格。答：先前**沒有**，已補。功能對測試案例對照共 40 案（A 已實作 19／B 承襲回歸 9／C 待開發 12）→ `docs/test-plan.html`。基準線：上游 155 個單元測試案例首次執行，全數通過零失敗。**兩項自訂功能仍零自動測試覆蓋**，補齊約需 2 天 | 🔄 規格已備妥待核准 | 核准後先寫 9 個「自動·電腦」案例＋手動檢查表 |
